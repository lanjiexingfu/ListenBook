...
public class Week extends RangeUnit {
    ...
    public Week(@NonNull LocalDate date, @NonNull LocalDate today, @Nullable LocalDate minDate,
        ...
        super(
                date, // 本周日
                date.plusWeeks(1).withDayOfWeek(6),  // 下周周六
                today,
                minDate,
                maxDate
        );
    ...
    public boolean select(@NonNull LocalDate date) {
        if (date != null && isIn(date)) {
            setSelected(true);

            for (Day day : mDays) {
                day.setChecked(day.getDate().isEqual(date));
            }
            return true;
        } else {
            return false;
    ...
    public void build() {
        ...
        for(LocalDate date = getFrom();
            date.compareTo(getTo()) <= 0;
            date = date.plusDays(1)) {
            Day day = new Day(date, isToday(date));
