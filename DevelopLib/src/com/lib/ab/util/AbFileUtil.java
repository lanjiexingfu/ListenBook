...
import com.lib.base.utils.BitmapUtil;
import com.lib.base.utils.CheckUtils;
import com.lib.base.utils.StrUtil;
...
public class AbFileUtil {
     ...
     *
     * @param url           文件的网络地址
     * @param type          图片的处理类型（剪切或者缩放到指定大小，参考BitmapUtil类）
    ...
    public static Bitmap getBitmapFromSD(File file, int type, int desiredWidth, int desiredHeight) {
        ...
        try {
            ...
            //文件存在
            if (type == BitmapUtil.CUTIMG) {
                bitmap = BitmapUtil.getCutBitmap(file, desiredWidth, desiredHeight);
            } else if (type == BitmapUtil.SCALEIMG) {
                bitmap = BitmapUtil.getScaleBitmap(file, desiredWidth, desiredHeight);
            } else {
                bitmap = BitmapUtil.getBitmap(file);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static Bitmap getBitmapFromURL(String url, int type, int desiredWidth, int desiredHeight) {
        Bitmap bit = null;
        try {
            bit = BitmapUtil.getBitmap(url, type, desiredWidth, desiredHeight);
        } catch (Exception e) {
            DLog.d(AbFileUtil.class.getSimpleName(), "下载图片异常：" + e.getMessage());
        }
        return bit;
    }
    ...
    public static Bitmap getBitmapFromSrc(String src) {
        ...
        try {
            ...
            DLog.d(AbFileUtil.class.getSimpleName(), "获取图片异常：" + e.getMessage());
        }
        return bit;
    }
    ...
    public static String checkLocalExitFile(String url, String dirPath) {
        //先判断SD卡中有没有这个文件，比较后缀部分比较
        String fileNameNoMIME = getCacheFileName(url);
        File parentFile = new File(dirPath);
        File[] files = parentFile.listFiles();
        for (int i = 0; i < files.length; ++i) {
            String localFileName = files[i].getName();
//            String name = localFileName.substring(0, localFileName.lastIndexOf("."));
            if (localFileName.equals(fileNameNoMIME)) {
                //文件已存在
    ...
    public static String downloadFile(String url, String dirPath) {
        ...
        try {
            ...
            String exitFile = checkLocalExitFile(url, dirPath);
            if (CheckUtils.isAvailable(exitFile)) {
                return exitFile;
            }
            ...
            e.printStackTrace();
            DLog.e(AbFileUtil.class.getSimpleName(), "有文件下载出错了,已删除");
    ...
    public static int getContentLengthFromUrl(String Url) {
        ...
        try {
            ...
            e.printStackTrace();
            DLog.d(AbFileUtil.class.getSimpleName(), "获取长度异常：" + e.getMessage());
        }
    ...
    public static String getRealFileNameFromUrl(String url) {
        ...
        try {
            ...
            e.printStackTrace();
            DLog.e(AbFileUtil.class.getSimpleName(), "网络上获取文件名失败");
        }
        return name;
    }
