...
public class XZip {
	...
	public static boolean zipFile(String inFilePath,String outFilePath){
		...
		try {
				DLog.d("zip", "path-->" + inFilePath);
				DLog.d("zip", "OUTPATH-->" + outFilePath);
				zipOutputStream = new ZipOutputStream(new FileOutputStream(outFilePath));
				File file = new java.io.File(inFilePath);		
				//压缩  
				ZipFiles(file.getParent()+java.io.File.separator, file.getName(), zipOutputStream);
		        //完成,关闭  
