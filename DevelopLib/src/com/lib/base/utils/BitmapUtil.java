...
import com.lib.base.utils.dct.FDCT;
...
public class BitmapUtil {
	...
	public static Bitmap getBitmap(String url, int type,
		...
		try {
			...
			DLog.d(BitmapUtil.class.getSimpleName(), "" + e.getMessage());
		} finally {
			try {
				if (is != null) {
					is.close();
	...
	private static boolean checkBitmap(Bitmap bitmap) {
		if (bitmap == null) {
			DLog.e(BitmapUtil.class.getSimpleName(), "原图Bitmap为空了");
			return false;
		}

		if (bitmap.getWidth() <= 0 || bitmap.getHeight() <= 0) {
			DLog.e(BitmapUtil.class.getSimpleName(), "原图Bitmap大小为0");
			return false;
		}
		return true;
	}
	...
	private static boolean checkSize(int desiredWidth, int desiredHeight) {
		if (desiredWidth <= 0 || desiredHeight <= 0) {
			DLog.e(BitmapUtil.class.getSimpleName(), "请求Bitmap的宽高参数必须大于0");
			return false;
		}
		return true;
	}
		...
		if (bitmap != null) {
			try {
				if (!bitmap.isRecycled()) {
					DLog.d(BitmapUtil.class.getSimpleName(),
...
//		if(width < GlobalConfigure.screenWidth){
